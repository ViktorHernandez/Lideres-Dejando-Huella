...
def request_adoption():
    ...
    cur = db.execute("INSERT INTO adoption_requests (user_id, animal_id, message) VALUES (?, ?, ?)",
...
def list_adoptions():
    ...
    if g.current_user["role"] == "admin":
        base = "SELECT ar.id, ar.user_id, u.name as user_name, ar.animal_id, a.name as animal_name, ar.message, ar.status, ar.created_at FROM adoption_requests ar LEFT JOIN users u ON ar.user_id = u.id LEFT JOIN animals a ON ar.animal_id = a.id ORDER BY ar.created_at DESC"
        params = []
    else:
        base = "SELECT ar.id, ar.user_id, u.name as user_name, ar.animal_id, a.name as animal_name, ar.message, ar.status, ar.created_at FROM adoption_requests ar LEFT JOIN users u ON ar.user_id = u.id LEFT JOIN animals a ON ar.animal_id = a.id WHERE ar.user_id = ? ORDER BY ar.created_at DESC"
...
def update_adoption_status(adoption_id):
    ...
    db.execute("UPDATE adoption_requests SET status = ? WHERE id = ?", (status, adoption_id))
    db.commit()
